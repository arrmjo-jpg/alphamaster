...
{
    /**
     * Let a provider row name the media analysis capability (ADR 0054).
     *
     * The sixth migration to rewrite this constraint, in the same shape as the ones before it:
     * the permitted values are written out rather than read from the enum, because a migration
     * is a historical snapshot.
     */
    private const CAPABILITIES_BEFORE = ['sms', 'captcha', 'ai', 'push', 'social_login', 'cdn'];

    /**
     * @var array<int, string>
     */
    private const CAPABILITIES_AFTER = ['sms', 'captcha', 'ai', 'push', 'social_login', 'cdn', 'media_analysis'];
    ...
    /**
     * Media analysis rows are removed first. Rolling back with one present would leave the
     * table holding a value its own constraint forbids, which PostgreSQL refuses outright and
     * SQLite accepts silently.
     */
    public function down(): void
    {
        DB::table('integration_usage_logs')->where('capability', 'media_analysis')->delete();
        DB::table('integration_providers')->where('capability', 'media_analysis')->delete();
