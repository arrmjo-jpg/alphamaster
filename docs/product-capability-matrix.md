...
| 11 | AI providers | **YELLOW** | `IntegrationCapability::AI`; `openai` (chat completions), `anthropic` (messages API, pinned version header) and `gemini` (`generateContent`) behind `AiManager`, each with its endpoint and authentication fixed in the driver, credentials encrypted on the row, seeded inactive. Proven with `Http::fake()` at the wire: for all three, the request reaches the vendor's own endpoint in its own shape, and a vendor error quoting the key is stored without it; for OpenAI, also a refusal, a timeout, an empty answer, an unreadable credential and a missing key; for Gemini, a refusal in its own words and an answer split across parts; and an unconfigured platform, for the capability as a whole. Anthropic's and Gemini's timeout, empty answer and missing key are not yet tested. **Missing: the same real-vendor call as row 10.** |
| 12 | AI settings | **GREEN** | `ai.max_output_tokens` (32–4096) and `ai.timeout_seconds` (5–300), each labelled and explained in both languages, each shown to change the request that reaches the vendor. The model is not a platform setting: each provider keeps its own, and `ai.translation_model` is removed (ADR 0044, amended 2026-09-11). No `ai.enabled` switch: a configured provider is what enables AI, so there is one control rather than two that can disagree. |
...
| 20 | Image processing | **RED** | `MediaProcessorContract` has `GenericFileProcessor` for images and documents and, since 2026-09-14, `TimedMediaProcessor` for video and audio metadata via ffprobe (ADR 0054 §11). The container has no `gd` or `imagick`: extensions are `pdo_pgsql, pgsql, pcntl, posix, bcmath, opcache, intl, zip, exif, redis, fileinfo`, so images are still not decoded, resized or watermarked. |
